...
// --- НОВО: СИГНАЛ КЪМ FACEBOOK ПИКСЕЛА ---
    ...
    try {
      // 2. Изпращаме данните фоново към Google
      fetch('https://script.google.com/macros/s/AKfycbxdfX08UpaXeSCSQrSJFl82hQQzFsGH7q5B3ZIdA98myswoJfNNdsJdRw215ard8qCHeA/exec', {
        method: 'POST',
        mode: 'no-cors',
        headers: {
      ...
      // 3. Изчистваме полетата, за да е готова формата за следващ път
  ...
  return (
    <section id="order" ref={sectionRef} className="py-8 lg:py-20 bg-[#FDFBF6] relative">
      
      {/* ИЗСКАЧАЩ ПРОЗОРЕЦ (MODAL) */}
      {orderSubmitted && (
        <div 
          className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all"
          onClick={() => setOrderSubmitted(false)}
        >
          <div 
            className="bg-white rounded-2xl p-8 lg:p-12 max-w-md w-full text-center shadow-2xl animate-in fade-in zoom-in duration-300"
            onClick={(e) => e.stopPropagation()} 
          >
            <div className="w-20 h-20 bg-[#E4F22B] rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-[#E4F22B]/20">
              <Check className="w-10 h-10 text-[#0C6D3E]" />
            </div>
            <h3 className="text-2xl lg:text-3xl font-bold text-[#0C6D3E] mb-4" style={{ fontFamily: 'Bricolage Grotesque, sans-serif' }}>
              Благодарим за поръчката!
            </h3>
            <p className="text-base lg:text-lg text-[#0C6D3E]/70 mb-8">
              Ще се свържем с вас за потвърждение в рамките на 24 часа.
            </p>
            <Button 
              onClick={() => setOrderSubmitted(false)} 
              className="btn-primary w-full py-6 text-lg rounded-xl"
            >
              Добре
            </Button>
          </div>
            ...
            <div className="bg-white rounded-xl lg:rounded-2xl shadow-lg p-4 lg:p-6">
              ...
              <ul className="space-y-2 text-xs lg:text-sm text-[#0C6D3E]/80">
                <li className="flex items-start gap-2">
                  <span className="text-[#E4F22B]">•</span> Доставка до 2 работни дни
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#E4F22B]">•</span> Наложен платеж (плащате при получаване)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#E4F22B]">•</span> Цената на доставката се заплаща отделно
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#E4F22B]">•</span> Доставка до избран от вас офис
                </li>
              </ul>
            </div>
          </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 lg:space-y-5">
                  ...
                  <div>
                    ...
                    <Textarea 
                      ...
                      placeholder="Например: Подходящо време за доставка..."
